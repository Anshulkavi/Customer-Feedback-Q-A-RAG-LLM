...
# Streamlit import (only failsafe import inside setup_database)
...
class RAGEngine:
    ...
    def setup_database(self):
        """Setup database connection (with Streamlit secrets fallback)."""
        try:
            ...
            logger.info(f"🔍 Loaded DB: {DB_USER}@{DB_HOST}:{DB_PORT}/{DB_NAME}")

            if not all([DB_USER, DB_PASS, DB_HOST, DB_PORT, DB_NAME]):
                raise ValueError("❌ Missing one or more DB credentials!")
            ...
            self.engine = create_engine(
                url, isolation_level="AUTOCOMMIT", connect_args={}
            )
    ...
    def load_reviews(self):
        """Load reviews from available tables."""
        try:
            table_queries = [
                "SELECT review_id, review_text, rating, sentiment_score, sentiment_label FROM public.facts_reviews LIMIT 1000",
                "SELECT review_id, review_text, rating, sentiment_score FROM public.facts_reviews LIMIT 1000",
            ...
            for query in table_queries:
                try:
                    ...
                    self.df["sentiment_score"] = self.df.get(
                        "sentiment_score", pd.Series([0] * len(self.df))
                    ).fillna(0).astype(float)
                    self.df["review_text"] = self.df["review_text"].fillna("").astype(str)
                    return
                except Exception as qerr:
                    logger.warning(f"⚠️ Query failed: {qerr}")
                    continue
    ...
    def build_index(self):
        ...
        try:
            ...
            embeddings = self.model.encode(
                review_texts, show_progress_bar=True, convert_to_numpy=True
            )
            ...
            self.id_map = {
                i: {
                    "review_id": self.df.iloc[i]["review_id"],
                    "text": self.df.iloc[i]["review_text"],
                    "rating": self.df.iloc[i].get("rating", 0),
                    "sentiment": self.df.iloc[i].get("sentiment_score", 0),
                }
                for i in range(len(self.df))
            }
    ...
    def generate_summary(self, query, reviews):
        ...
        try:
            ...
            Based on the following customer reviews, answer the user's query.
            ...
            Summarize patterns, insights, and key sentiments under 200 words.
            """
            ...
            response = model.generate_content(
                prompt,
                generation_config=genai.types.GenerationConfig(
                    temperature=0.7, max_output_tokens=300, top_p=0.9
                ),
            )
            ...
            logger.error(f"❌ Summary generation failed: {e}")
            return f"Sorry, I couldn’t summarize this. Error: {e}"


# ✅ Streamlit cache: prevents reload on every rerun
@st.cache_resource
def get_rag_engine():
    return RAGEngine()
...
def query_reviews(query, top_k=5):
    return get_rag_engine().query_reviews(query, top_k)


def generate_ai_summary(query, reviews):
    return get_rag_engine().generate_summary(query, reviews)
